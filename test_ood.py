...
parser.add_argument('--test_dir', default='Test_Folder/E1', type=str)
...
def main_worker():
    ...
    log_file = os.path.join(log_dir, 'Test-OOD-' + args.experiment + '-' + args.time + '.txt')
    log_args(log_file)
    ...
    grouping = {'G1': ['E01-08', 'E02-10'], 'G2': ['E06-16'], 'G3': ['E07-26', 'E08-25', 'E09-20', 'E10-23', 'E11-18', 'E13-28', 'E14-27']}

    if args.skip_infer:
        logging.info('Skipped inference, try using existing inference for test.')
    else:
        test_folder = os.path.join(os.path.abspath(args.root), args.test_dir, 'img')
        output_folder = './test/pred/E1'
        logging.info(f'OOD inferencing...')
    ...
    for group in ['G1', 'G2', 'G3']:
        ...
        target_folder = os.path.join(os.path.abspath(args.root), args.test_dir, 'labelcol')
        output_folder = './test/pred/E1'
        names = [i + '.nii.gz' for i in grouping[group]]
        val_dice = []
        val_nsd = []
        for name in names:
    ...
    logging.info(f'\nAll Average OOD testing DICE {sum(dices)/len(dices):.5f}, NSD {sum(nsds)/len(nsds):.5f}')
    torch.save({'dice': dice_dict, 'nsd': nsd_dict}, f'./test/OOD_{local_time}.pth')
